class Game { //<>// //<>// //<>// //<>// //<>//
  ...
  int w;
  int h;
  int numberOfEnemies;// antaller af enemies. 
  int numberOfHealth; //antallet af grønsagerne.
  int playerLife;
  int player2Life;
  int[][]board;// dobbelt array / nest in a nest
  Dot player;// den blå spiller.
  Dot player2;// den lyserøde spiller.
  //Det skal være array da der skal være mere end en. 
  Dot[] enemies;// det er et array så det kan ændres.
  Dot[] health;// de "sunde" grønsager, samme gældendene her med array. 
  ...
  // konstruktør
  // arbejder med class i konstruktøren.
  Game(int gW, int gH, int nE, int nH) {
    w = gW;
    h = gH;
    numberOfEnemies = nE;
    numberOfHealth = nH; //min
    enemies = new Dot[nE];
    health = new Dot[nH]; //min
    playerLife=100;
    player2Life=100;//min
    board  = new int[gW][gH];
    player = new Dot(0, 1, w-1, h-1);
    player2 = new Dot(10, 15, w-1, h-1);//min
    keys = new Keys();
...
//kalder vi på i main SPI_1_hele
  void update() {
    updatePlayer();
    updateEnemies();
    updateHealth(); 
  ...
  void updatePlayer() {
    ...
    //update player 2
    // tjekke om spilleren har trykket på piltasterne. 
    if (keys.akuDown) {
      player2.moveUp();
    }
